...
users.forEach(function (item) {
    var _el = createElement(item)
    document.getElementById("users").appendChild(_el);
})
...
// ContactName'i A harfi ile başlayanları consola yazdır.

let customers = users.filter(x => x.contactName.toLocaleLowerCase().startsWith('a'));
console.log('A harfi ile başlayanlar customerlar. ', customers);

// Country'si UK olanları consola yazdır.

let uk = users.filter(x => x.address.country == 'UK');
console.log("Customerların countrysi uk olanlar.", uk);

//Customerları CompanyName'e göre sıralama.

let sortedCompanyName = users.sort(function (a, b) {
    return a.companyName.localeCompare(b.companyName)
});
console.log("CompanyName'e göre sıralama. ", sortedCompanyName);

//İd'si "DUMON" olanı consola yazdır object olarak.

let user = users.find(x => x.id == "DUMON");
console.log("ID'si 'DUMON' olan", user);


//Telefon Numarası 1 ile başlayanları consola yazdır.

let startWithOne = []
users.forEach(function (item) {
    for (let i = 0; i < item.address.phone.length; i++) {
        let char = item.address.phone.charAt(i)
        if (isNumber(char)) {
            if(char == '1')
            {
                startWithOne.push(item)
            }
            break;
        }
    }
...
console.log('Phone 1 ile baslayanlar', startWithOne)

//Telefon Numarası formatlama consola yazdır.
...
users.forEach(function (item) {
    ...
    item.address.phone = newPhone
})

console.log('Telefon numarası formatlama işlemi sonrası liste', users)

//isNuber gönderilen karakterin sayı olup olmadığını kontrol eden bir fonksiyondur.
